...
$status_filter = $_GET['status'] ?? 'menunggu_konfirmasi_pembayaran'; // Default filter
...
} elseif ($status_filter === 'diproses_penjual') { // Jika sudah dikonfirmasi pembayaran
    $sql .= " AND py.status_pembayaran = 'confirmed' AND p.status = 'diproses_penjual'";
} elseif ($status_filter === 'dikirim') { // Jika sudah ditandai dikirim oleh seller
    $sql .= " AND p.status = 'dikirim'";
} elseif ($status_filter === 'selesai') { // Jika sudah dikonfirmasi pembeli
    $sql .= " AND p.status = 'selesai'";
} elseif ($status_filter === 'dibatalkan') { // Jika pesanan dibatalkan
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($status_filter === 'menunggu_konfirmasi_pembayaran'): ?>
                                <a href="aksi_pesanan_seller.php?action=confirm_payment&pesanan_id=<?= $row['pesanan_id'] ?>&status_filter=<?= $status_filter ?>" class="btn btn-success btn-sm" onclick="return confirm('Konfirmasi pembayaran ini?')">Konfirmasi</a>
                                <a href="aksi_pesanan_seller.php?action=reject_payment&pesanan_id=<?= $row['pesanan_id'] ?>&status_filter=<?= $status_filter ?>" class="btn btn-danger btn-sm" onclick="return confirm('Tolak pembayaran ini?')">Tolak</a>
                            <?php elseif ($status_filter === 'diproses_penjual'): ?>
                                <a href="aksi_pesanan_seller.php?action=mark_as_shipped&pesanan_id=<?= $row['pesanan_id'] ?>&status_filter=<?= $status_filter ?>" class="btn btn-primary btn-sm" onclick="return confirm('Tandai pesanan ini sebagai dikirim?')">Tandai Dikirim</a>
                            <?php endif; ?>
